"""Validated market history loading service."""

import pandas as pd

from app.core.constants import MINIMUM_HISTORY_DAYS
from app.data.cache import MarketDataCache
from app.data.schema import OHLCV_COLUMNS, normalize_ohlcv
from app.providers.base import MarketDataError, MarketDataProvider


class DataLoader:
    """Load provider data and enforce scanner input invariants."""

    def __init__(
        self,
        provider: MarketDataProvider,
        period: str = "2y",
        cache: MarketDataCache | None = None,
    ) -> None:
        """Initialize the loader with an injected market data provider."""
        self._provider = provider
        self._period = period
        self._cache = cache

    def load(self, symbol: str) -> pd.DataFrame:
        """Return validated, deduplicated, chronological OHLCV bars."""
        normalized_symbol = symbol.strip().upper()
        if not normalized_symbol:
            raise ValueError("Symbol must not be blank")
        namespace = self._provider.cache_namespace
        frame = (
            self._cache.get(namespace, normalized_symbol, self._period)
            if self._cache is not None
            else None
        )
        cache_miss = frame is None
        if frame is None:
            frame = normalize_ohlcv(
                self._provider.history(normalized_symbol, self._period), normalized_symbol
            )
            frame = frame.loc[:, list(OHLCV_COLUMNS)]
        if len(frame) < MINIMUM_HISTORY_DAYS:
            raise MarketDataError(
                f"{normalized_symbol} has {len(frame)} rows; {MINIMUM_HISTORY_DAYS} required"
            )
        if (frame[["Open", "High", "Low", "Close"]] <= 0).any().any():
            raise MarketDataError(f"{normalized_symbol} contains non-positive prices")
        if (frame["Volume"] < 0).any():
            raise MarketDataError(f"{normalized_symbol} contains negative volume")
        if cache_miss and self._cache is not None:
            self._cache.put(namespace, normalized_symbol, self._period, frame)
        return frame
